...
def deconv3d_bn_block(in_channels, out_channels, use_upsample=(2, 2, 2), kernel=4, stride=2, padding=1, momentum=0.01,
                      activation=ACTIVATION, norm='batch'):
    if use_upsample:
        print('USING SHIT 11111111111111')
        print('USING SHIT 11111111111111')
        print('USING SHIT 11111111111111')
        up = nn.Sequential(
...
class Generator(nn.Module):
    def __init__(self, n_channels=1, out_channels=1, nf=32, norm_type='batch',
                 activation=ACTIVATION, final='tanh', mc=False, use_upsample=False):
        ...
        print('use upsample:', use_upsample)
