...
def tif_to_patches(npys, **kwargs):
    ...
    for i in range(len(npys)):
        ...
        if kwargs['percentile'] is True:
            p0 = np.percentile(npys[i], 0.1)
            p1 = np.percentile(npys[i], 99.9)
...
if __name__ == "__main__":
    root = '/media/ghc/Ghc_data3/BRC/aisr/aisr122424/'
    suffix = ''
    npy0 = tiff.imread(root + 'roi0A.tif')
    ...
    tif_to_patches([npy0],
                   ...
                   trd=(None, ), norm=('11', ),
                   prefix='', ftr=-10, zrescale=None, percentile=True)
